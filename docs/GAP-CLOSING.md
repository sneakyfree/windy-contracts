...
| **windy-search** | `~/windy-search` | ✅ **healing reads SHIPPED 2026-07-13** (#61): /ops/logs + /ops/config + /ops/selftest (canary via the router's real path, 300s cache — metered bridges), EPT-gated; manifest 5→8 tools. 🔴 Grant-gated redeploy. Remaining: mutating pair (port the windy-mind ops-hook #61 pattern) + check_for_update | cleanest leaf — replicated the Mind template in one pass. |
| **windy-mail** | `~/windy-mail` | ✅ **healing reads SHIPPED 2026-07-13** (#84): /ops/logs (NOTHING message-derived can enter — by construction) + /ops/config (booleans) + /ops/selftest (Stalwart/db/redis legs, 60s cache), EPT-or-Pro-JWT; manifest 4→7 tools. 🔴 Grant-gated redeploy. Remaining: FULL canary send (needs dedicated canary mailbox), mutating pair (ops-hook #61 port), check_for_update | privacy hard line's sharpest platform — handled. |
| **windy-clone** | `~/Windy-Clone` | ✅ **healing reads SHIPPED 2026-07-13** (#58): /ops/logs + /ops/config (surfaces DEV_MODE — an auth bypass if on in prod) + /ops/selftest (dummy order resolve+price, NO purchase — proven side-effect-free); manifest 3→6 tools. 🔴 Grant-gated redeploy. ⚠️ pre-existing main failure flagged: test_version_in_openapi_spec (KeyError 'paths'). Remaining: mutating pair (ops-hook #61 port), check_for_update | replication #3 — recurring-four READ passes now COMPLETE fleet-wide. |
...
| **windy-admin** | `~/windy-admin` | ✅ **healing reads SHIPPED 2026-07-13** (#26): /ops/logs (strictly meta) + /ops/config (booleans + token COUNT) + /ops/selftest (real envelope→ingest→query canary — proves the whole fleet's telemetry pipeline), staff-gated; manifest 3→6 tools. 🔴 Grant-gated redeploy. Remaining: mutating pair (ops-hook #61 port) | Fleet-version PUBLISHER already DONE (`/v1/fleet-versions`). **With this, EVERY Class-C read gap in the fleet is closed.** |
